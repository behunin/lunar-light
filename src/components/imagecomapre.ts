...
class ImageCompare {
  ...
  private arrowAnimator: Array<any>;
  private arrowCoordinates;
  private active;
  private slideWidth;
  private lineWidth;
  constructor(el: HTMLDivElement, settings = {}) {
    const defaults = {
      controlColor: "#FFFFFF",
      controlShadow: true,
      addCircle: false,
    ...
    if (this.animationFrameId === null) {
      this.animationFrameId = requestAnimationFrame(() => {
        ...
        if (ev.type === "mousemove" || "mousedown") {
          ...
          x = mouse.clientX - bounds.left;
          y = mouse.clientY - bounds.top;
        }
        if (ev.type === "touchmove") {
          const touch = ev as TouchEvent;
          x = touch.touches[0].clientX - bounds.left;
          y = touch.touches[0].clientY - bounds.top;
        }
        ...
        if (position >= 0 && position <= 100) {
          ...
          if (this.settings.fluidMode) {
            this.settings.verticalMode
              ? (this.wrapper!.style.clipPath = `inset(0 0 ${
                  100 - position
                }% 0)`)
